...
def readDepList(filter_id, user, passwd):
    ...
    for result in results:
        try:
            ...
            # Append if the field does not exist
            depList.append(jira_url + 'browse/' + result.key + ' - ' +
                           result.fields.summary)
...
def writeConfig():
    ...
    # Set permissions on file to read/write only for current user
    # prevents mis-use of the values stored here (security)
    os.chmod(getConfigPath(), 0o600)
